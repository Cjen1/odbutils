...
module Persistant (P : Persistable) = struct
  ...
  let of_path path =
    let rec read_loop t reader =
      ...
      | `Eof ->
          return t
      | `Ok op ->
          read_loop (P.apply t op) reader
    in
    let%bind t =
      match%map
        try_with (fun () -> Reader.with_file path ~f:(read_loop @@ P.init ()))
      with
      | Ok v ->
          v
      | Error _ ->
  ...
  let write t op = Writer.write_bin_prot t.writer P.bin_writer_op op

  let datasync t = Writer.fdatasync t.writer

  let close t = Writer.close t.writer
end
